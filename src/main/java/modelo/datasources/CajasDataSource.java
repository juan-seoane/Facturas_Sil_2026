...
public class CajasDataSource implements JRDataSource {
    ...
    private List<EntradaCaja> listaCajas = new ArrayList<EntradaCaja>();
    ...
    public Object getFieldValue(JRField jrf) throws JRException {
        ...
        if ("ID".equals(jrf.getName())) {
            ...
            valor = listaCajas.get(indiceCajaActual).getFecha().toString();
        } else if ("DEBE".equals(jrf.getName())) {
            if (listaCajas.get(indiceCajaActual).isHaber()) valor = 0.00; else valor =
                (-1) * (listaCajas.get(indiceCajaActual).getCaja());
        } else if ("HABER".equals(jrf.getName())) {
            if (listaCajas.get(indiceCajaActual).isHaber()) valor =
                listaCajas.get(indiceCajaActual).getCaja(); else valor = 0.00;
    ...
    public void addCaja(EntradaCaja cj) {
        listaCajas.add(cj);
    }
}
