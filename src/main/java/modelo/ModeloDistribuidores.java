...
public class ModeloDistribuidores {
    ...
    public static ModeloDistribuidores getModelo() {
        if (instancia == null) instancia = new ModeloDistribuidores();
        return instancia;
    }
    ...
    public boolean autosave(String ruta) {
        ...
        if (auto.escribir(listaDistribuidores)) return true; else return false;
    }
    ...
    public ArrayList<RazonSocial> filtrarDistribuidores(ArrayList<RazonSocial> lista) {
        ...
        if (TablaDistribuidores.filtrosactivos()) {
            ...
            if (ControladorDistribuidores.filtros.getChbFiltroCategoria().isSelected()) {
                FiltroCategoriaRS filtro2 = new FiltroCategoriaRS(
                    ControladorDistribuidores.filtros.getCmbCategoriasFiltros().getSelectedItem().toString()
                );
                lista3 = filtro2.filtrar(lista2);
            } else lista3 = lista2;
            if (ControladorDistribuidores.filtros.getChbFiltroDistribuidor().isSelected()) {
                FiltroDistribuidorRS filtro3 = new FiltroDistribuidorRS(
                    ControladorDistribuidores.filtros.getFiltroDist()
                );
    ...
    public boolean comprobacionRS() {
        ...
        if (FormularioFact.getFormulario().getLetraNIFRS().equals("")) {
            ...
            nuevoNIF =
                new NIF(
                    FormularioFact.getFormulario().getNumNIFRS(),
                    FormularioFact.getFormulario().getLetraCIFRS().toUpperCase(),
                    isCIF
                );
            ...
            nuevoNIF =
                new NIF(
                    FormularioFact.getFormulario().getNumNIFRS(),
                    FormularioFact.getFormulario().getLetraNIFRS().toUpperCase(),
                    isCIF
                );
            if (!comprobacionNIF(nuevoNIF)) {
                insertarDesdeFactura(nuevoNIF, FormularioFact.getFormulario().getRS());
    ...
    public boolean insertarDesdeFactura(NIF nuevoNIF, String razon) {
        RazonSocial nuevoDistribuidor = new RazonSocial(
            this.getListaDistribuidores().size() + 1,
            nuevoNIF,
            razon,
            new Nota(0, "-")
        );
