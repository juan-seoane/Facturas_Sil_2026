...
public class FiltroConceptoCaja implements Filtro {
    ...
    public List filtrar(List lista) {
        ...
        for (Object entrada : lista) {
            if (
                (((Caja) entrada).getOrigen().toUpperCase().trim()).contains(this.origen.toUpperCase().trim())
    ...
    public boolean filtrar(int index, List lista) {
        EntradaCajaentrada = (Caja) lista.get(index);
        if ((entrada.getOrigen().toUpperCase().trim()).contains(this.origen.toUpperCase().trim())) return true;
        return false;
    }

    public Object filtrar(Object f) {
        if ((((Caja) f).getOrigen().toUpperCase().trim()).contains(this.origen.toUpperCase().trim())) return f;
        return null;
    }
}
