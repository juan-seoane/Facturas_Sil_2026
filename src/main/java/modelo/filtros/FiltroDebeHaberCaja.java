...
public class FiltroDebeHaberCaja implements Filtro {
    ...
    public List filtrar(List lista) {
        ...
        for (Object entrada : lista) {
            try {
                if (((Caja) entrada).isHaber() == isHaber) lista2.add((Caja) entrada);
            } catch (Exception e) {}
        }
        return lista2;
    }
    ...
    public boolean filtrar(int index, List lista) {
        ...
        if (((Caja) entrada).isHaber() == isHaber) return true; else return false;
    }

    public Object filtrar(Object c) {
        if (((Caja) c).isHaber() == isHaber) return c;
