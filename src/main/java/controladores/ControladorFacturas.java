...
public class ControladorFacturas extends Thread {
    ...
    public static synchronized ControladorFacturas getControlador(FxCntrlTablaFCT fxcntrfct) {
        if (instancia == null) {
            System.out.println(
                "[ControladorFacturas>getControlador(fxcontr)] Instancia vacia, creando una nueva instancia generica con GUI asociada!"
            );
            try {
                instancia = new ControladorFacturas();
            } catch (InterruptedException | BrokenBarrierException e) {
                System.out.println("Error al recoger la instancia del ControladorFacturas");
            }
        }
        GUIon = true;
        //ANCHOR - 24-07-14 : FxCntrlTablaFCT
    ...
    public synchronized TableView<Factura> getTableViewFCT() {
        if (this.tableViewFCT == null) this.tableViewFCT = new TableView<Factura>();
        ...
        tabla
            ...
            .addListener((obs, oldSelection, newSelection) -> {
                if (newSelection != null) {
                    ...
                    System.out.println(
                        "[FxCntrlTablaFCT>listener1] Has seleccionado el INDEX: " +
                        FxCntrlTablaFCT.getIndiceActual() +
                        " en la TABLAFCT"
                    );
                    System.out.println(
                        "[FxCntrlTablaFCT>listener1] ...desde el hilo " + Thread.currentThread().getName()
                    );
                    ((TableView<Factura>) (getFXcontrlTablaFCT().getTableView())).getSelectionModel()
                        .selectedIndexProperty()
                        .addListener(
                            (ObservableValue<? extends Number> observable, Number oldValue, Number newValue) -> {
                                int ind = newValue.intValue();
                                ControladorFacturas.facturaActual =
                                    ((TableView<Factura>) (getFXcontrlTablaFCT().getTableView())).getSelectionModel()
                                        .getSelectedItem();
                                //Colocar index en el lblIndex de la tablaFCT
                                FxCntrlTablaFCT.getFxController().setIndiceActual(ind);
                                System.out.println(
                                    "[FxCntrlTablaFCT>listener2] Has seleccionado el INDEX: " +
                                    FxCntrlTablaFCT.getIndiceActual() +
                                    " en la TABLAFCT"
                                );
                                System.out.println(
                                    "[FxCntrlTablaFCT>listener2] ...desde el hilo " + Thread.currentThread().getName()
                                );
                            }
                        );
                    ...
                    System.out.println(
                        "[FxCntrlTablaFCT>initialize listener] Parece que no se ha detectado la factura seleccionada, a pesar de haber recogido el evento"
                    );
                }
            });
    }
        ...
        while (true) {
            ...
            if (FXcontrlTablaFCT != null && FXcontrlTablaFCT.HaCambiado()) {
                ...
                switch (FXcontrlTablaFCT.getPulsado()) {
                    ...
                    case 3:
                        System.out.println(
                            "[ControladorFacturas>run] Se comienza a editar la factura seleccionada. TableView->" +
                            tableViewFCT.hashCode()
                        );
                        int index = FXcontrlTablaFCT.getIndiceSeleccionadoTabla();
                        Factura f = FXcontrlTablaFCT.getFacturaSeleccionadaTabla();
                        if (mostrarVisorFCT(index, f)) editarFacturaVisor();
                        break;
                    case 4:
                        System.out.println(
                            "[ControladorFacturas>run] Se borra la Factura actual -> " +
                            FxCntrlTablaFCT.getFxController().getIndiceSeleccionadoTabla() +
                            " - TableView->" +
                            tableViewFCT.hashCode()
                        );
                        borrarFacturaTabla();
                    ...
                    case 5:
                        System.out.println(
                            "[ControladorFacturas>run] Se muestra la Ventana de Filtros. TableView->" +
                            tableViewFCT.hashCode()
                        );
                        break;
                    case 6:
                        System.out.println(
                            "[ControladorFacturas>run] Se imprime la TablaFCT. TableView->" + tableViewFCT.hashCode()
                        );
                        break;
                }
            ...
            if (FXcontrlVisorFCT != null && FXcontrlVisorFCT.HaCambiado()) {
                ...
                switch (FXcontrlVisorFCT.getPulsado()) {
                    case 1:
                        if (FxCntrlVisorFCT.modo == FxCntrlVisorFCT.VISOR) {
                            try {
                                ocultarVisorFCT();
                                System.out.println(
                                    "[ControladorFacturas>run>visorSwitch] Cerrando el Visor de Facturas"
                                );
                            } catch (InterruptedException | BrokenBarrierException e) {
                                e.printStackTrace();
                            }
                        } else if (FxCntrlVisorFCT.modo == FxCntrlVisorFCT.EDITAR) {
                            if (acabarEdicionFacturaVisor()) System.out.println(
                                "[ControladorFacturas>run>visorSwitch] Factura Editada!"
                            );
                        }
                        break;
                    case 2:
                    ...
                    case 3:
                        System.out.println(
                            "[ControladorFacturas>run] Se activa la Edicion del VisorFCT para la Factura actual"
                        );
                        editarFacturaVisor();
                        break;
                    case 4:
                        ...
                        break;
                    case 5:
                        if (anteriorFacturaVisor()) System.out.println(
                            "[ControladorFacturas>run] La seleccion de la tablaFCT se movera hacia arriba"
                        );
                        break;
                    case 6:
                        if (siguienteFacturaVisor()) System.out.println(
                            "[ControladorFacturas>run] La seleccion de la tablaFCT se movera hacia abajo"
                        );
                        break;
                }
        ...
        Platform.runLater(
            new Runnable() {
                ...
                public void run() {
                    ...
                    System.out.println(
                        "[ControladorFacturas>anteriorFacturaVisor] Leida Lista de Facturas de tamaño " + lista.size()
                    );
                    if (i > 0) {
                        Platform.runLater(
                            new Runnable() {
                                ...
                                public void run() {
                                    ...
                                    System.out.println(
                                        " [ControladorFacturas] Actualizando tabla - tableView " +
                                        tv.hashCode() +
                                        " - nuevo index: " +
                                        nuevoindice
                                    );
        ...
        Platform.runLater(
            new Runnable() {
                ...
                public void run() {
                    ...
                    List<Factura> lista = ModeloFacturas.leerFacturasSinFiltrar();
                    System.out.println(
                        "[ControladorFacturas>siguienteFacturaVisor] Leida Lista de Facturas de tamaño " + lista.size()
                    );
    ...
    public boolean actualizarTabla(int sel) {
        ...
        alert.initModality(Modality.APPLICATION_MODAL);
        if (visorFCT.isShowing()) alert.initOwner(visorFCT); else alert.initOwner(tablaFCT);
        // Mostrar la alerta y esperar a que el usuario la cierre
        alert.showAndWait();
...
*/
        ...
        if (FXcontrlTablaFCT == null) {
            Platform.runLater(
                new Runnable() {
                    ...
                    @Override
                    public void run() {
                        FxmlHelper loader = new FxmlHelper(_Ruta.FXML.getRuta() + "/fxmltablaFCT.fxml");
                        Parent root;
                        ...
                        System.out.println(
                            "[ControladorFacturas] Controlador FX para tabla de FCT asignado: cntrFXtabla->" +
                            FXcontrlTablaFCT.hashCode()
                        );
                        ...
                        System.out.println(
                            "[CFCT>cargarTablaFacturas] Stage de Tabla con hashcode :" + tabla.hashCode()
                        );
                        ControladorFacturas.tablaFCT = tabla;

                        System.out.println(
                            "[CFCT>cargarTablaFacturas] Stage de Tabla asignado a ControladorFacturas.tablaFCT con valor : StageTabla->" +
                            tablaFCT.hashCode() +
                            " - tblVw->" +
                            contrFxtablaTemp.getTableView().hashCode()
                        );
                        //ANCHOR - 24-07-14 : Asignar contrFX de T/FCT a CFCT
                        if (FXcontrlTablaFCT == null) {
                            System.out.println(
                                "[CFCT>cargarTablaFacturas] El contrlFX de la tablaFCT es NULL. El programa se cierra!!!"
                            );
                            System.exit(0);
                        }
                        //ANCHOR - tableView
        ...
        Platform.runLater(
            new Runnable() {
                ...
                public void run() {
                    ...
                    System.out.println(
                        "[controladorFacturas>mostrarTablaFacturas] Mostrando Stage de tablaFCt " + tablaFCT.hashCode()
                    );
                }
            }
        );
    ...
    public synchronized void ocultarTablaFacturas() {
        System.out.println(
            "[ControladorFacturas>ocultarTablaFacturas] Se oculta  el StageTabla->" + tablaFCT.hashCode()
        );
        ...
        Platform.runLater(
            new Runnable() {
                ...
                public void run() {
                    System.out.println(
                        "[ControladorFacturas>ocultarTablaFacturas>runLater] Oculto el StageTabla->" +
                        tablaFCT.hashCode()
                    );
    ...
    public synchronized boolean cargarVisorFacturas() {
        if (visorFCT == null) {
            Platform.runLater(() -> {
                Stage visor;
                FxmlHelper loader = new FxmlHelper(_Ruta.FXML.getRuta() + "/visorFormFCT.fxml");
                Parent root;
                root = loader.cargarFXML();
                ...
                visor.setOnCloseRequest(ev -> FxCntrlVisorFCT.getFxController().getVisorFCT().hide());
                ...
                System.out.println(
                    "[ControladorFacturas>cargarVisorFacturas] asignado ContrlFact.visorFCT: " +
                    ControladorFacturas.visorFCT.hashCode()
                );
            });
            return true;
        } else {
    ...
    public synchronized boolean mostrarVisorFCT(int index, Factura f) {
        ...
        if (f == null) {
            System.out.println(
                "[ControladorFacturas>mostrarVisorFCT] No hay factura seleccionada. No se muestra el visor"
            );
            return false;
        }
        ...
        Platform.runLater(() -> {
            ...
            System.out.println(
                "[ControladorFacturas>mostrarVisorFCT>runLater] index: " + index + "\nfactura: \n" + f.toString()
            );
            ...
            System.out.println(
                "[ControladorFacturas>mostrarVisorFCT>runLater] se muestra el visorFCT del contrFX con hashCode: " +
                FxCntrlVisorFCT.getFxController().getVisorFCT().hashCode()
            );
        });
        return true;
    ...
    public void imprimirInfoTabla() {
        ...
        System.out.println(
            "ControladorFacturas]   tableview->" + ((tableViewFCT == null) ? "NULL" : tableViewFCT.hashCode())
        );
        System.out.println("ControladorFacturas]  Stagetabla->" + ((tablaFCT == null) ? "NULL" : tablaFCT.hashCode()));
        System.out.println(
            "ControladorFacturas] cntrFxtabla->" + ((FXcontrlTablaFCT == null) ? "NULL" : FXcontrlTablaFCT.hashCode())
        );
        System.out.println(
            "ControladorFacturas] listaFXfact->" +
            (
                (ModeloFacturas.getModelo().getListaFXFacturas() == null)
                    ? "NULL"
                    : ModeloFacturas.getModelo().getListaFXFacturas().hashCode()
            )
        );
        if (FXcontrlTablaFCT != null) FXcontrlTablaFCT.imprimirInfoTabla(); else System.out.println(
            "----No hay datos del cntFXtabla*---"
        );
