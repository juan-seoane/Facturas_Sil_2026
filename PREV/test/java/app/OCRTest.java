...
import infrastructure.src.main.java.com.sil.facturas.infrastructure.debug.Debug;
...
public class OCRTest {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(modelosDir, "modelo_*.json")) {
            ...
            for (Path modeloPath : stream) {
                ...
                resultado.forEach((campo, texto) -> {
                    ...
                    Debug.print(campo + " → " + texto);
                });

                Debug.print("=== Modelo OK ===");
            }
        }
    }
    ...
    public void testFacturaDesdeImagen() throws Exception {
        ...
        // 1. Cargar modelo OCR
        ModeloOCRParser parser = new ModeloOCRParser();
        ModeloOCR modelo = parser.parse(
                new File("./data/datos/modelosOCR/modelo_test1A.json"));
    ...
    public void testVisualizarModeloOCR() throws Exception {
        ...
        ModeloOCRParser parser = new ModeloOCRParser();
        ModeloOCR modelo = parser.parse(new File("data/datos/modelosOCR/modelo_test1B.json"));
        Debug.print("[OCRTest>testVisualizarModeloOCR] Lista de zonas:");
        modelo.zonas().forEach((s, roi) -> Debug.print(s +" -> " + roi));
    Debug.print(
        ...
        ModeloOCRVisualizer.dibujarModelo(
                modelo,
                imagenFactura,
                salida
        );
    ...
    public Map<String, String> aplicarOCR(ModeloOCR modelo, OCRService ocrService) throws Exception {
        ...
        Debug.print("\n=== Imagen del modelo ===");
        Debug.print("Ruta imagen: " + imgFile.getAbsolutePath());
        Debug.print("Existe: " + imgFile.exists());
        Debug.print("Tamaño: " + imgFile.length());
        Debug.print("\n----------------------------");
        Debug.print("ENCODING TEST = " + System.getProperty("file.encoding"));
